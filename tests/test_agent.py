...
def test_notify_invalid_fields_sends_mail(agent, monkeypatch, raw_output_factory, form_factory):
    sent = {}
    ...
    agent._notify_invalid_fields(raw, form_factory(type_document="CNI"))
...
def test_notify_invalid_fields_no_mail_when_all_valid(agent, monkeypatch, raw_output_factory, form_factory):
    calls = []
    monkeypatch.setattr(agent_module.mailService, "send_email", lambda **k: calls.append(k))
...
def test_notify_invalid_fields_swallows_mail_errors(agent, monkeypatch, raw_output_factory, form_factory):
    def boom(**kwargs):
...
async def test_process_end_to_end(agent, monkeypatch):
    ...
    monkeypatch.setattr(
        agent_module.insightface_engine,
        "compare",
    ...
    mail_calls = []
    monkeypatch.setattr(agent_module.mailService, "send_email", lambda **k: mail_calls.append(k))
